...
| 5 | **Doktor Adaptasyon** | ✅ **Self-signup** (`/kayit` — Google[env-gated]/Apple[yakında]/e-posta → `User`+`Doctor` `verified:false`, `lib/doctor-signup` + `lib/oauth`) + **Doktor Ana Sayfası — pencere-tabanlı** (v6.42 düzeni: Bildirim Tercihi / Eşleşen Vakalar / **Uzaktan Sağlık** [eski "Klinik Nöbet" penceresi; 3'lü nöbet tercihi pencere içinde "Klinik Nöbet" bölümü] / İkinci Görüş / Sağlık Turizmi / Ücretsiz Sağlık Hizmeti / Konsültasyon Talepleri [**tam genişlik**, alt alta]), her doktora ünvan+opt-in'e göre koşullu (`lib/doctor-home.ts`). **Haberler ARTIK PENCERE DEĞİL (v6.46-47):** ana sayfadan çıktı → üst bant linki + `/doktor/haberler` sayfası; **canlı PubMed beslemesi** (E-utilities; branş→MeSH `NEWS_QUERIES`, son 1 yıl, `fetchBranchNews`) — branş yayınları ÖNCE, sonra genel gündem; kartta gerçek dergi künyesi + yazarlar + **gerçek DOI/PubMed bağlantısı** + koddan üretilen kapak (branş sembolü + 3px kulvar şeridi; dış görsel CSP'de yasak). Başlık/özet İngilizce gelir → `Translation` önbelleğiyle TR'ye çevrilir (6 sn bütçe; aşılırsa özgün dil, önbellek arkada dolar). Besleme erişilemezse kartlar **örnek içerik** diye işaretlenir ve bağlantı VERİLMEZ (`lib/medical-news.ts`; ⚠️ sayfada `fetchCache = "default-cache"` ŞART — `force-dynamic` tek başına fetch'i no-store'a düşürür, her açılış NCBI'ya gidip ~10 sn sürer). **v6.41 birleşik vaka listesi:** Eşleşen Vakalar = Case satırları + atanan İkinci Görüş + açık Konsültasyon talepleri tek kuyrukta, hasta kartı diliyle (3px kulvar şeridi + mono etiket, 5. kulvar `--lane-consult` **sarı** [v6.42; indigo ayrışmıyordu]); sayaçlar (Toplam/Bekleyen/Acil) tıklanır collapse+filtre, liste varsayılan kapalı + **tam-genişlik tek-tuş "Tüm eşleşen vakaları göster"** (v6.42); kulvar çipleri + sıralama (Aciliyet/En yeni); üst bant nav'ından "Ücretsiz Hizmet" linki kaldırıldı (rota+panel durur). **Kulvar ikonları hasta↔doktor ORTAK SET (v6.42):** telehealth=HeartPulse · so=Stethoscope · tourism=Plane · free=HeartHandshake · consult=Inbox. **Kurumsal giriş kapısı (v6.42):** rol seçici altında hasta kapısıyla aynı sağlayıcı kompozisyonu (Google `intent=doctor` — kayıtlı e-posta kendi rolüyle girer · Apple · E-posta) + **ilk-giriş onboarding** (`/doktor/baslangic`: **FHIR uzmanlık** [diploma/tescil no = Practitioner.identifier + uzmanlık belgesi = qualification] + **branş işlemleri & ücretleri** ≥1 [`ProcedureSelector`→`Doctor.procedures`] + **zorunlu mesleki belge** — Tıp Diploması + MMSS poliçesi; hepsi tamamlanmadan hesap **aktifleşmez** [`canCompleteOnboarding` gate; `DoctorDocument` + `Doctor.activatedAt` + `lib/doctor-activation`; içerik at-rest şifreli; MMSS teminat limiti → M3 Katman 3 malpraktis girdisi]; İkinci Görüş ünvana göre; Ücretsiz Sağlık Hizmeti + Konsültasyon opt-in) + Panel 1 yalnız eşleşen vakalar + itibar/hakediş/kapasite/profil tercihleri (dil/pazar/işlem-ücret/opt-in). **Doğrulama kapısı:** self-signup doktor `verified:false` başlar → doktor dizini + Nöbetçi/İcapçı/Ücretsiz Sağlık Hizmeti eşleştirmelerinde gizli; **`/admin/hekim-onay`** (ADMIN/Etik Kurul) onayıyla `verified:true` olur (bildirim) |
