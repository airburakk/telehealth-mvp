...
| `npx tsx scripts/fix-pubmed-dates.ts [--prod] [--yaz]` | **PubMed yayın tarihi onarımı** (v6.85: PubMed'in `pubdate`/`sortpubdate` alanı derginin KAPAK tarihidir — sürekli-yayın dergileri makaleyi "31 Aralık"a, aylık dergiler gelecek sayıya yazar; gerçek tarih `epubdate`. Mevcut `NewsArticle` kayıtlarını esummary'den yeniden hesaplar, toplayıcının KENDİ `pubDate()`'iyle [tek doğruluk kaynağı]. Dry-run varsayılan · dokunulan tek alan `publishedAt` · idempotent · dev'de koşuldu 2026-08-06: 74/74 düzeltildi, gelecek tarihli 71→0) |
