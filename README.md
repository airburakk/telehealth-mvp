...
**🚀 Canlı: https://auraglobalcare.com** (AURA kanonik domaini, v6.201 · 2026-09-02; eski `telehealth-mvp-roan.vercel.app` host'u da 200 döner, canonical yeni domaini gösterir) · Demo girişi: `doktor@air.test` / `1234`
...
| `npx tsx scripts/merge-congress-research.ts --dizin <klasor> [--yaz]` | araştırma ajanı çıktısını (`*-sonuc.json`) küratörlü kaynağa **birleştirir** (v6.121). Üç sessiz hatayı kilitler: dokunulmayan kayıt kaybolmaz · yeniden adlandırılan hayalet bırakmaz · çok-branşlı satırlar eşitlenir. **Kimlik koruması:** ada yıl veya sondaki parantez eklenmesi reddedilir (yoksa her tur yeni satır doğar). **Aggregator süzgeci:** kongreuzmani/10times/waset vb. `sourceUrls`'ten düşer, `officialUrl`'e sızmışsa null'lanır. Doğrulama çıktısı gören dosyayı ATLAR |
| `npx tsx scripts/apply-congress-verification.ts --dizin <klasor> [--yaz]` | doğrulama turu hükümlerini (`hukum`/`duzeltmeler`) kaynağa işler (v6.121); doktor-yüzü alanlardan iç süreç notunu ayıklar. YALNIZ `hukum` taşıyan dosyayı kabul eder (merge ile iki yönlü kilit) |
| `npx tsx scripts/prune-congress-orphans.ts [--prod] [--yaz]` | ad değişince DB'de kalan **hayalet satırları** toplar (v6.121); `CongressFollow` kayıtlarını halefe TAŞIR. 🔴 **seed'den SONRA koşulur** — sıra bekçisi kaynağın tarihli kimliklerinin %90'ı DB'de değilse DURUR (prod'da yakalandı: halef havuzu eksikken ESPRAS→ASPS %62 yanlış eşleşmesi). Silme eşiği 0.8 (yeniden adlandırmanınki 0.6 — asimetrik risk) |
| `npx tsx scripts/ingest-ttb-events.ts [--prod] [--yaz] [--from=YYYY-MM] [--to=YYYY-MM]` | **TTB akredite etkinlik ingest'i** (v6.120: kredilendirme.ttb.dr.tr ay ay taranır — yanıt 50 kayıtta tavan yapar; test kaydı elenir; tür kod önekinden okunur [KNG/SMP/KRS…]; branş eşlemesi sözlük taramasıyla; idempotent, dry-run varsayılan) |
...
| `npx tsx scripts/probe-membership-stamps.ts [--dev]` | üyelik damgaları **salt-okur sayım** (2026-09-12: `verified`↔`diplomaVerifiedAt` bağımsızlığı [v6.196 kapısının teyidi] + `studentTrack` damgasız öğrenci izi + `LandingEvent` "ogrenci" yerleşimi/section_view payı; envanter §4 kalemleri için tekrar koşulabilir. `PROD_DATABASE_URL` açıkça şart [DATABASE_URL fallback'i bilinçli yok], `--dev` Neon development branch provası; `src/lib/db.ts` import etmez → kod-seviyesi `AURA_DB_GUARD`'a takılmaz — PHI/e-posta basmaz, yalnız sayı) |
| `npx tsx scripts/rotate-kek.ts [--apply] [--blobs]` | **KEK rotasyonu** (2026-07-17; **v6.273 2026-09-18: ŞEMA-GÜDÜMLÜ ortak motor** `lib/kek-rotation` — envanter YOK: information_schema'daki tüm base tabloların tüm metin kolonlarında `enc:v1:`/`blob:v1:` taranır, `id` cursor'uyla yeniden sarılır; eski 13 kolonluk elle envanter koddaki ~40 şifreli kolonun çoğunu kaçırıyordu. İçerik çözülmez, yalnız DEK sarımı; dry-run varsayılan; `ROTATE_NEW_KEK` [+ `ROTATE_OLD_KEK`]; prod `ALLOW_PROD_KEK_ROTATION=1` şart; foreign/döndürülemeyen satırda exit 1. Break-glass eşleniği: `/admin` → "Şifreleme anahtarı" [`/api/admin/kek-rotate`, `KEK_ROTATION_SECRET` ile kurulur]) |
...
| 3 | **Sağlık Turizmi** | ✅ Tier'lı paket, dinamik fiyat, **3 kademeli sigorta** (1 zorunlu · 2 operasyon teminat poliçesi [toplam fatura×oran×branş riski] · 3 malpraktis — doktorun yüklediği MMSS'inin bıraktığı boşluğu doldurur; `lib/pricing.ts` `computeInsurance`, parametrik/endikatif) + **hasta sağlık beyanı → risk çarpanı** (v6.31: `/paket`'te hasta-only form [kronik/ilaç/sigara/ameliyat, triyaj+profil prefill'li] → `computeHealthRiskMult` çarpımsal-tavanlı, yalnız Katman 2/3'e; beyan `Case.healthDeclaration` **şifreli+vaka-sabit** + `User.patientHealthHistory` profil kopyası; personel/acente ham beyan GÖRMEZ, yalnız çarpan; prim client+server aynı fonksiyondan), **lojistik Patient Journey takibi** (durum+tarih+not; koordinatör yönetir, hasta görür) + hastaya teklif gönderme (link/PDF); `api/ai/package-proposal` (SOAP'tan AI paket teklifi) ucu **v6.290'da kaldırıldı** — hiçbir arayüz çağırmıyordu, fiyat motoru `lib/pricing computePackage` yerinde. ⚠️ **Sağlık turizmi kulvarı ÖDEMESİZ (2026-07-23, kullanıcı kararı):** teklif onayı ödeme/escrow İÇERMEZ — `tourism` bayrağıyla `OfferView`/`ReservationView` escrow bandı+split GÖSTERMEZ, buton "Teklifi Onayla", backend `escrowStatus` PENDING kalır (HELD yazılmaz; respond+booking route). Escrow+split **Branş Doktoru kulvarındaki** paket akışında yaşamaya devam eder |
...
**Doctorium ÜÇ KATMANLI ÜYELİK (v6.232–v6.245, 2026-09-05 — 🚀 CANLI; deneme bayrağı iki Vercel projesinde AÇIK):** kitle
...
yer imleri ve gönderilmiş bildirim href'leri kırılmaz. Admin paneli `/admin/etkinlik`.
🔴 **"Otomatik kaynak YOK" varsayımı çürüdü:** dernek siteleri makine-okunur takvim yayımlamıyor ama
**kredilendirme otoritesi yayımlıyor** — TTB STE/SMG akredite etkinlik kaydı (`kredilendirme.ttb.dr.tr`)
kimliksiz, açık ve sorgulanabilir (`scripts/ingest-ttb-events.ts`). Küratörlü veri İKAME EDİLMEDİ:
`source` değerleri ayrı (`curated` ↔ `ttb-kredilendirme`), unique çakışmaz, ikisi bir arada yaşar.
Şema (`20260819140000`): `eventType` (9 tür Türkçe slug — kongre·sempozyum·kurs·egitim·konferans·
calistay·seminer·atolye·diger) · `ttbCode` (TTB'nin kendi kodu birebir) · `scope`'a üçüncü değer
`uluslararasi-katilimli` · `@@index([eventType,startDate])`. Aynı migration `Doctor.feedModules`
JSON'unda `"kongre"` → `"etkinlik"` **göçünü yapar** — `parseFeedModules` fail-open olduğu için
göç edilmeseydi o bölümü seçmiş doktor onu HATASIZ kaybederdi.
**Arayüz:** tür çipleri (çoklu seçim + "Hepsi", `?t=`), varsayılan **kongre + sempozyum** (kurs TTB
kaydının en büyük kovası — süzgeçsiz bırakılsa kongre gürültüde kaybolurdu); Akışım'ın 3 kartlık
etkinlik kotası da varsayılan türlerle sınırlı; kart + detayda tür rozeti ve TTB akreditasyon satırı.
⚠️ **Kredi SAYISI hiçbir yerde yazılmaz** — TTB kodu "akredite" der, "şu kadar puan" DEMEZ: puan
katılım süresine + Kurul kararına bağlı oluşur ve TTB programında üst/alt oturumlar ayrı satır
olduğu için süreden türetmek de yanıltıcı olurdu. Dayanak + kredi kuralları (EK-1: 40 dk = 1 kredi,
...
zemini. ⚠️ **v6.144'te KALDIRILDI:** dış takvime aktarım (`.ics`) — iki ayrı "takvime ekle" deneyimi
(dosya indir vs uygulama-içi takip) kafa karıştırıyordu (kullanıcı bildirimi). Etkinlik kartındaki
"Takvime ekle" düğmesi artık `CongressFollow`'u açan AYNI mekanizmayı kullanıyor (`FollowButton`
"action" varyantı) — dosya üretmez, etkinlik doğrudan BU iç takvime düşer.
...
- **Uygulama-katmanı at-rest şifreleme (E2EE Faz 1):** hassas klinik kolonları (belge içeriği, transkript,
  ...
  gerektiğinde çözer → defense-in-depth (DB-dump + KEK'siz operatör). `DATA_ENCRYPTION_KEK` yoksa üretimde
  yazım **fail-closed** (dev/test'te düz metin). KMS swap-point hazır. (`lib/crypto.ts`)
  **KEK rotasyonu (v6.273, 2026-09-18):** motor şema-güdümlü (`lib/kek-rotation` — envanter yok, tüm metin
  kolonları taranır; betik `scripts/rotate-kek.ts` + **break-glass ucu** `/api/admin/kek-rotate`: Vercel'deki
  KEK geri okunamadığından insan-okur kopya kaybında yeni anahtara sunucu içinde rotasyon; `KEK_ROTATION_SECRET`
...
- **Doctorium Post** — abone doktora günlük özet gazetesi (`daily-digest` cron'u 06:30 TR; opt-in kanal, e-posta/app).
...
**v6.263 (2026-09-10) — hukuki set Paket 2 (05 madde 3.1b/3.10, 06 madde A.2/B.2):** `Doctor.lastLoginAt`
(`lib/login-activity.ts recordLogin` tek dokunuş noktası) + `lib/abandoned-sweep.ts` — doğrulanmış (diploma/
öğrenci) ama son girişten **3 yıl + 30 gün bildirim** sonra terk edilmiş hesap silinir (klinik bağ/PENDING
belge fail-closed, `purgeAbandonedAccount` — `purgeTrialAccount` deseni). `AccessLog.purgedAt` +
`lib/audit.ts purgeStaleAuditMeta` — **2 yıl sonra ip/userAgent boşaltılır**; `verifyAccessChain`'e
`ConsentRecord.purgedAt` İLE AYNI "bağ-koruyan" mühür-atlama deseni (`purgedSeals` sayacı) — naif bir UPDATE
zinciri KIRIK gösterirdi, bu desen onu önler. `lib/kvkk-applications.ts` + `/doctorium/kvkk-basvuru` formu
(`LegalShell` artık `children` alır) + `/admin/kvkk-basvurulari` yönetim ekranı — **KVKK m.11 başvuru kütüğü**,
...
**v6.300 (2026-09-22) — 7-C düzeltmesi: tek birimli hukuki çeviri SKALER araçla:** üretim ısıtmasında her koşuda aynı 4 birim eksik kalıyordu (koşullar 5.1/5.5/6.1 + çerez hücresi; Almanca/Bulgarca): dizi şemasında model uzun paragrafı ("→" adımları, (a)/(b)/(c) sıralaması) 2–3 öğeye bölüyor → "sayı uyuşmuyor" → çeviri atılıyor; birim-birim yedek de tek öğede aynı nedenle KALICI başarısızdı. Parçaları birleştirmek denendi ve reddedildi (bölünme noktasındaki tırnaklar düşüyor, " ." artığı). Çözüm `translateLegalBatch`: tek öğe → `translateLegalSingle` (`submit_translation`, `translation: string` — model bölemez, noktalama korunur; ham metin gönderilir); çok öğe dizi şemasında kalır (kısmi doldurma yine YOK). DEV Bulgarca koşullar 110→113/113 doğrulandı. Isıtma betiği sayacı dürüst: "özgünle aynı kalan N (dil adları/özel adlar dâhil)" — eskiden özdeş dizeler "eksik" görünüyordu. Test: `legal-translate-batch` (5). 🪤 `.aura-preview/` (başka oturumun yapı artığı, git-ignore'lu) `tsconfig include "**/*.ts"` yüzünden yerel tsc/build'e girer — temiz worktree'de build alınır.

**v6.286 (2026-09-20) — 7-C hukuki çeviri ONAY KUYRUĞU (👤 karar: rozet dili "İncelenmiş çeviri"):** TR/EN dışı 9 dildeki hukuki belge çevirileri hastaya **"Otomatik çeviri (yapay zekâ). Henüz hukuki incelemeden geçmedi."** rozetiyle gösterilir (belge kabuğu çeviri kutusunun ilk satırı + onam kapısında bölüm başı; TR/EN kanonik görünümde rozet yok). ADMIN `/admin/hukuki-ceviri` kuyruğunda (Yönetim dizini kartı) belge × dil × kanonik sürüm başına TR kanonikle **yan yana** inceler; "Onayla" → metin **DONDURULUR** (`LegalTranslationApproval.markdown`, migration `20260920190000_legal_translation_approval`) ve rozet **"İncelenmiş çeviri. Hukuki inceleme tarihi: <tarih>"** olur; onam kapısındaki aydınlatma/koşullar aynı onayı kullanır (`ConsentRecord.shownTextHash` = dondurulmuş metnin hash'i → hastanın incelenmiş metni okuduğu sonradan kanıtlanır; mühür formülü değişmedi). TR kaynak hash'i ya da `AURA_LEGAL_VERSION` değişince onay **kendiliğinden eskir** (sayfa otomatiğe döner, kuyruk "Onay eskidi"); "Onayı kaldır" `revokedAt`. Uç `POST /api/admin/hukuki-ceviri` (`action: approve|revoke|generate`; onay adminin GÖRDÜĞÜ `textHash` ile — sunucu önbellekten yeniden kurar, değiştiyse **409**; eksik/önbelleksiz çeviri onaylanamaz; yalnız ADMIN; audit `LEGAL_TRANSLATION_APPROVE|REVOKE|GENERATE`; `maxDuration 120`). Kuyruk ve inceleme **önbellekten** okur (`peekLegalTranslations`, Claude'a istek yok); üretim "Çeviriyi üret" düğmesi ya da `scripts/translate-legal.ts` / **`scripts/translate-legal-prod.mjs`** (üretim; `.env` `AURA_DB_GUARD=block` ise kabukta bilinçli `warn`). 🐛 Tamlık ölçütü düzeltildi: özgün metne **özdeş** çeviri ("Vercel", "`session`" gibi özel ad/kod) artık "eksik" sayılmaz (Rusça aydınlatma yanlışlıkla %96 "eksik" görünüyordu); başarısız birim haritaya yazılmaz. Kapsam dışı (bilinçli): çeviri düzenleme (yalnız onay/ret), AI rıza metinleri (arayüz çeviri hattı), Doctorium belgeleri (TR). Testler: `legal-approval` · `legal-approval-admin` · `legal-translate`.

**v6.285 (2026-09-20) — Paket 7 hukuki metin TAM LOKALİZASYON (👤 karar A):** hukuki belgeler + hasta onam kapısı + KVKK formu artık hastanın seçtiği dilde (11 arayüz dili). Kaynak DAİMA TR kanonik markdown → `lib/legal-markdown-split` (başlık/liste/blockquote/tablo/hr çeviri DIŞI; paragraf = birim) → `lib/i18n getLegalTranslations` (Translation önbelleği, `legal:` ad alanı `legalHash`; hukuki prompt `translateLegalBatch`; chunk 8 öğe/2400 kar., sayı uyuşmazlığında birim-birim yedek; düşen birim TR kalır, önbelleğe yazılmaz) → `lib/legal-translate translateLegalMarkdown` (`complete=false` = kısmi; motor yoksa null → EN kanonik). Gösterim dili `lib/aura-legal/display resolveLegalDisplay(?lang=<kod|ad>, hastaProfilDili)`: tr → TR · en → EN · diğer → **EN kanonik + o dilde bilgilendirme çevirisi** (`auraLegalLang()` TR/EN dışı tanınan dilde `en` döner); belge sayfaları çeviri görünümünde **noindex**, kabuk "bağlayıcı metin Türkçe (ikincil İngilizce)" notu + kanonik bağlantı + dil seçici (Türkçe · English · diğer diller), ar/fa rtl; KVKK formu etiketleri useT. Hasta onam kapısı (`/onam`): TR/EN dışı dilde çeviri BİRİNCİL okuma, her bölümün altında kanonik EN açılır; onay POST'u `lang:"en"` (kanonik hash) + `shown:{lang, aydinlatmaHash, kosullarHash}` → `parseShownTranslation` → **`ConsentRecord.shownLang/shownTextHash`** (migration `20260920150000_consent_shown_translation`; mühür formülü v2 DEĞİŞMEDİ — ispat eki; kanıt sayfası "Okuduğunuz dil" + hash satırı). Ön-ısıtma: `npx tsx scripts/translate-legal.ts [--lang=Rusça]` (ilk hasta beklemesin; DEV .env — ÜRETİM önbelleği için DATABASE_URL'i PROD_* ile AÇIKÇA ver). Sözlükler saf modüllerde (`lib/aura-consent-gate-ui`, `lib/aura-legal/shell-ui`) — "use client" modülünden veri export'u RSC'de proxy olur. Testler: `legal-markdown-split` (5 belge yapısal gidiş-dönüş) · `legal-display` · `consent-shown-translation`. Sonraki adım (👤): C — `/admin` çeviri onay kuyruğu ("otomatik" → "onaylı çeviri").
...
**v6.221 (2026-09-04) — hukuki belgeler revizyon turu 1 (👤 avukat):** sözleşme madde başlıkları BÜYÜK HARF (02), iç atıflarda "§" yerine "madde N" (tüm belgeler), 02 madde 1.3 (Taraf/Taraflar tanımı), 01 madde 2 kutusu tek cümle. Belge sürümü **1.1 · 4 Eylül 2026** (`DOCTORIUM_LEGAL_VERSION`), beş yayın kesiti yeniden kesildi; `DOCTORIUM_CONSENT_VERSION` 1→2 — hash'lenen metin değiştiği için mevcut Doctorium üyeleri ilk girişte bir kez yeniden onaylar (kayıtlar zincirde kalır). Kaynak: vault `output/doctorium-hukuki-belgeler` (çalışma .md; teslim .docx/.pdf `_teslim-uret.py`).
...
| `/doctorium/aydinlatma` · `/doctorium/kosullar` · `/doctorium/cerez` · `/doctorium/icerik-politikasi` · `/doctorium/kvkk-basvuru` | **Doctorium hukuki belgeleri (v6.210, 2026-09-03)** — tek kaynak `lib/doctorium-legal/` (bkz. "Doctorium — ayrı ürün → Hukuki belgeler"); açık kabuk (`LegalShell`), AURA kromu yok, canonical `doctorium.tr`, Doctorium sitemap'inde. **Platform içi KVKK başvuru formu CANLI (v6.263)** — yalnız oturum açmış üyeye açık, `POST /api/doctorium/kvkk-basvuru`; `/admin/kvkk-basvurulari`'nde (ETHICS/ADMIN) yanıtlanır |
| `/aydinlatma` · `/kosullar` · `/cerez` · `/kvkk-basvuru` (· `/tele-saglik` bayrakla kapalı) | **AURA hukuki belgeleri (kod Paket A, v6.268, 2026-09-13)** — tek kaynak `lib/aura-legal` (vault `output/aura-hukuki-belgeler` Sürüm 1.0 NİHAİ; `texts/*.ts` vault'taki `_yayin-kesiti.py` ile üretilir, elle düzenlenmez); TR kanonik + `?lang=en` İngilizce ikinci kanonik (S4; çelişkide TR esastır; canonical TR, hreflang tr/en); vitrin kabuğu `AuraLegalShell` (V2Nav + AuraClosing, gövde açık `.aura-light` şeridi; `CHROME_FREE_ROUTES`); footer "Hukuki" grubu 9 dil; sitemap yalnız yayımlı rotalar; `AURA_ONLY_PREFIXES` (doctorium.tr → AURA 307). Kimlik alanları tüzel kişilik kurulana dek "AURA platform işleticisi" yer tutucusu; e-posta kanalı kutu açılana dek yayımlanmaz (platform içi form). `/kvkk-basvuru`: A07 bölüm A metni + oturumlu üyeye (hasta + personel) başvuru formu (`POST /api/kvkk-basvuru` = Doctorium işleyicisi, ortak `KvkkApplication` kütüğü). `/tele-saglik`: `published:false` → 404 + noindex (A03 yayın şartı: klinik hizmet sağlayıcı kimliği + USHAŞ yetki belgesi alanları). Hasta kayıt formunda aydınlatma bağlantısı. Kod Paket B (ekran = hash, `GENERAL_KVKK` v4 TR+EN) bu metin modülünü tüketir. Sözleşme testi `tests/unit/aura-legal.test.ts`. |
...
| `/doktor` (+`/baslangic`, `/profil-tamamla`, `/vaka/[id]`, `/takip`, `/profil`, `/ucretsiz-saglik`, `/konsultasyon`) | Doktor Ana Sayfası (pencere-tabanlı, v6.41 birleşik vaka listesi), ilk-giriş onboarding (**v6.87'den beri iki aşamalı** — Aşama 1: 🔄 v6.124'ten beri e-Devlet doğrulamalı diploma → yalnız Doctorium; Aşama 2: klinik havuz — diploma TEKRAR İSTENMEZ, yerine güvenlik katmanları v6.126), **OAuth profil-tamamlama ara sayfası** (v6.87 — Google/Apple hesabı branş/şehir boş açılır, bu ekran doldurtur), kokpit, izleme, profil, Ücretsiz Sağlık Hizmeti, klinik nöbet, Konsültasyon Talepleri kutusu |
...
| `/ogrenci` | **Tıp/Diş Hekimliği öğrencisi kapısı (v6.147'de mekanizma değişti):** doktor girişinden AYRI salt-kayıt sayfası (`StudentGateForm`; noindex; vitrin footer'ından 9 dilde link) + dürüst kapsam kutusu — haber/kongre/hukuk/kütüphane AÇIK · sponsor/anket/ödül + klinik yüzeyler KAPALI olduğu açıkça yazılır. Kayıtta üniversite+bölüm seçilir; e-posta seçilen üniversitenin bilinen öğrenci uzantısıyla (`lib/universities.ts` — 124 YÖK-tanınırlıklı üniversite: TR+KKTC+AZ+KZ+MK) eşleşmezse 400 ile reddedilir, eşleşirse `Doctor.studentTrack:true` hesap açılır ve doğrulama bağlantısı tıklanınca (`verify-student-email`) Doctorium erişimi açılır — eski belge yükleme (STUDENT_CERT, hiç gerçek doğrulama yapmıyordu) kaldırıldı |
...
| `/admin/ai-kullanim` | **AI kullanım sayacı (ADMIN, v6.298):** `AiUsageDaily` gün (UTC) × özellik × model çağrı/token toplamları + tahminî USD (fiyat listesi tarihli; Console esas). İçerik/PHI yok; her Anthropic çağrısı `lib/ai-usage.trackedCreate` ile sayılır, Gemini oturumu adet. Sayaç fail-open. |
...
| `digest` · `doctor/digest` | **Doctorium Post (v6.159):** `digest/unsubscribe` — e-posta tek-tık çıkış (GET=onay sayfası [istemci link ön-yüklemesi işlem YAPMAZ] · POST=RFC 8058; oturumsuz, audit'li `DIGEST_UNSUBSCRIBE`). ⚠️ **Token v6.198'de DB'ye taşındı** (`Doctor.digestUnsubToken` — doktor başına rastgele + kalıcı): eskiden `HMAC(SESSION_SECRET)` idi, ama bülten AURA projesinden gönderilip bağlantı doctorium.tr'de doğrulanıyor ve **iki projenin sırrı FARKLI ölçüldü** (2026-09-02) → çıkış 403 verecekti; DB ortak olduğu için env paritesi ihtiyacı kalktı. Bağlantı TABANI da `SITE_URL` değil `DOCTORIUM_CANONICAL_URL` (v6.197 — Doctorium markalı bülten AURA host'una göndermesin) · `doctor/digest` — abonelik kanalı tercihi (null/app/email; self-auth, feed-modules deseni) |
| `social-digest` | **Kamuya açık gazete seçkisi (v6.190):** GET — Bearer `SOCIAL_DIGEST_TOKEN` (env yoksa dormant 503, yanlış anahtar 401); 6 akıştan 1'er başlık (TEASER — Post'un tam hâli kamuya çıkmaz), gün-deterministik branş rotasyonu (aynı TR günü aynı yanıt — çift paylaşım kazasına kapalı), kişisel veri YOK. Tüketici: Hetzner `doctorium-n8n` sunucusundaki sabah akışı (07:45 → kart-render servisi → `bulten-arsiv/`; detay hafıza `hetzner-n8n-sunucu`). **Akademik + ilaç başlıkları ingest'te Türkçe doğar** (2026-09-02, `lib/translate-news` — Claude ile toplu çeviri, fail-open + hiza güvenliği; birikmiş kayıtlar `scripts/translate-titles-backfill.ts` ile çevrildi: 410 akademik + 305 ilaç). **v6.206 (2026-09-02 gece): ÖZET GİRİŞİ de Türkçeleşir** — başlık çevrilip özet İngilizce kalıyordu (kart/Post/bülten ona basıyordu); ayrı cron `api/cron/translate-news` (05:40 TR, `lib/translate-summaries`; kapsam `lib/news-language.needsSummaryTranslation` = akademik + ilaç + İngilizce sektörel) `summary`=Türkçe giriş (~700 kar., cümle sınırı — `summaryLead`), `summaryOriginal`=özgün tam metin yazar; yeni→eski, birikmişi gecelik bütçeyle kendisi kapatır (PROD backfill script'i yok); AI özetleri (`ensureClinicalSummary`/`ensureRegulationSummary`) kaynak olarak özgün metni okur. **v6.202 (2026-09-02):** sektörel İngilizce kaynaklar (Medscape / Medical Xpress / WHO) da ingest'te çevrilir — kapı tek yerde `lib/news-language.needsTitleTranslation` (Türkçe doğan kaynaklar geçmez; sözleşme testi `news-language.test`); seçkide **sektörel akış yerli kaynağı önceler** (`pickSocialDigest` — yerli yoksa yabancı kalır); RG ana sayfa fihrist artığı ("…İLÂN BÖLÜMÜ") `parseGazetteTodayHtml` (başlık yalnız `<a>` içinden) + `stripGazetteSectionSuffix` ile düzeltildi, `scripts/ingest-tr-sources.ts`'e idempotent "başlık onarımı" adımı eklendi. **🔁 v6.288 (2026-09-21, 👤 kural EKİ — tekrar yerine donör):** "her akıştan 1" korunur; bir akışta O GÜN yeni başlık yoksa (son 24 saatte akışa düşen yok — `SOCIAL_FRESH_MS`, tazelik ekseni `createdAt`) dünkü başlık TEKRARLANMAZ, yuva o gün en çok yeni başlık gelen akıştan (eşitlikte akış sırası; donör tükenince sıradaki) kullanılmamış taze bir başlıkla dolar — yuva sırası korunur, başlık donör akışın etiketini taşır, `items[].replaces` hangi akışın yerine girdiğini söyler; donör yoksa eski başlık `stale:true` ile kalır (boş kart yerine fail-soft), 48 saatte hiç içeriği olmayan akış yine düşer (doldurulmaz); `items[].id` + yanıt `fill{replaced,stale}` eklendi (alan EKLEME — kart tarafı eski alanları aynen görür). Durumsuz (dünkü seçki DB'den okunmaz); birim testi 16 sözleşme |
...
| `doctor` · `auth` | Doktor tercihleri/akademik/işlem · oturum + **`signup`** (doktor kaydı) + **`signup-staff`** (2026-08-12 — PARTNER/AGENCY/HEALTH_PRO başvurusu: şifreli yanıt + `STAFF_APPLICATION_KVKK` onamı + yetkisiz hesap) + **`google/{start,callback}`** (OAuth, env-gated) + **`forgot-password` · `reset-password`** (v6.194 — parola kurtarma; oturumsuz, yetkiyi TOKEN taşır: hesap keşfine KAPALI [hesap var/yok AYNI yanıt], token DB'de sha256 + TTL **1 saat** + tek kullanımlık, başarıda oturum AÇILMAZ ve TÜM oturumlar düşer, `emailVerifiedAt` de damgalanır [giriş ucu doğrulanmamışı reddettiği için ŞART, süs değil], e-posta kanalı dormant'ken 503 `channelDormant` → "gönderildi" demek yerine dürüst uyarı) |
...
| `doctor/verify/[channel]` | **AŞAMA 2 doğrulama OTP'si (v6.126):** `sms` · `work-email` — POST kod üretir+gönderir (kanal env'siz simülasyon), PUT kodu doğrular → `Doctor.smsVerifiedAt`/`workEmailVerifiedAt`. Self-auth (hedef doktor DAİMA oturumdan; IDOR yok) + rate-limit 5/10dk + audit. Kod sha256(kod+satırId) olarak saklanır, düz metin HİÇBİR yere yazılmaz |
| `admin/doctors/[id]/clinic-phone` | **Klinik telefonu geri-arama teyidi (v6.126):** koordinatör/ADMIN, HealthTürkiye'deki resmî tesis numarasından doktoru arayıp POST ile damgalar (`CLINIC_PHONE_VERIFY` audit) · DELETE geri alır. İnsan-işletimli — otomasyon bilinçli YOK |
