// ÜRETİLMİŞ YAYIN KESİTİ — kaynak: output/doctorium-hukuki-belgeler/02-uyelik-sozlesmesi-kullanim-kosullari.md (Sürüm 1.1 · 04.09.2026 — revizyon turu 1, 👤 nihai).
// Elle düzenleme: kaynak .md → vault; buradaki metin yayın kesitidir (iç notlar/karar bölümleri yok).
// Kimlik alanları (unvan/adres/MERSİS/KEP/VERBİS) tüzel kişilik kurulunca doldurulur (Kılavuz §8).
export const KOSULLAR_MD = `## 1. TARAFLAR VE KONU

**1.1.** İşbu sözleşme, **Doctorium platform işleticisi** ("**Platform**") ile
Doctorium'a üye olan gerçek kişi ("**Üye**") arasında, Üye'nin Doctorium'a erişimi ve kullanımına
ilişkin şartları düzenler.

**1.2.** Üye, kayıt sırasında bu sözleşmeyi onaylamakla hükümlerini kabul etmiş sayılır.

**1.3.** Platform ve Üye, işbu sözleşmede bundan böyle ayrı ayrı "**Taraf**" ve birlikte "**Taraflar**" olarak anılacaktır.

---

## 2. TANIMLAR

| Terim | Anlamı |
|---|---|
| **Doctorium** | Doğrulanmış doktorlara ve tıp öğrencilerine açık mesleki çalışma alanı |
| **Doktor Üye** | Mesleki kimliği e-Devlet mezun belgesi ile doğrulanmış hekim |
| **Öğrenci Üye** | Üniversite e-postası doğrulanmış tıp fakültesi öğrencisi |
| **İçerik** | Platformda sunulan haber, akademik yayın özeti, mevzuat, içtihat, doktrin künyesi, prospektüs bilgisi, kongre ve etkinlik kaydı |
| **Sponsorlu İçerik** | "Sponsorlu" etiketiyle yayımlanan, bedeli karşılığı yer verilen içerik |
| **Puan** | Platform içi, parasal değeri bulunmayan katılım göstergesi |

---

## 3. ÜYELİK ŞARTLARI VE DOĞRULAMA

**3.1.** Doctorium **kapalı bir platformdur**; üyelik yalnız aşağıdaki kişilere açıktır:
   a) Türkiye'deki bir tıp fakültesinden mezun olup tıp doktoru unvanına sahip kişiler,
   b) Tıp fakültesi öğrencileri.

**3.2. Doktor Üye doğrulaması.** Üye, mesleki kimliğini e-Devlet üzerinden alınan **barkodlu
mezun belgesi** ile doğrular. Doğrulama tamamlanmadan içerik erişimi açılmaz.

**3.3. Öğrenci Üye doğrulaması.** Üye, üniversitesine ait e-posta adresinin doğrulanmasıyla
kaydolur. Öğrenci üyeliğinin kapsamı ve sınırları ayrıca düzenlenmiştir.
→ Tıp Öğrencisi Üyeliği Ek Metni (öğrenci kayıt akışında gösterilir)

**3.4.** Üye, doğrulamaya esas beyan ve belgelerin **doğru, güncel ve kendisine ait** olduğunu
taahhüt eder. Gerçeğe aykırı beyan, madde 10 uyarınca üyeliğin sona erdirilmesi sebebidir.

**3.5.** Üyelik **kişiseldir**; hesap bilgileri üçüncü kişilerle paylaşılamaz, devredilemez.

**3.6.** Mesleki kimlik doğrulaması bugün yalnız e-Devlet mezun belgesi üzerinden yapılmaktadır;
yabancı ülkede alınmış diploma bu aşamada doğrulanamaz ve üyelik kapsamı dışındadır. Platform,
ileride başka doğrulama yolları açabilir; açıldığında bu madde güncellenir.

---

## 4. HİZMETİN NİTELİĞİ VE SINIRLARI

**4.1. Bilgi hizmeti.** Doctorium, mesleki bilgiye erişimi kolaylaştıran bir **derleme ve
sunum** hizmetidir.

**4.2. Tıbbi tavsiye değildir.** Platformda yer alan hiçbir içerik; tanı, tedavi, ilaç
seçimi veya herhangi bir klinik karar için **tavsiye niteliği taşımaz.** Klinik kararların
sorumluluğu münhasıran Üye'ye aittir.

**4.3. Hukuki tavsiye değildir.** Mevzuat, içtihat ve doktrin bölümlerinde sunulan içerik
bilgilendirme amaçlıdır; hukuki görüş veya tavsiye yerine geçmez.

**4.4. Üçüncü kişi kaynakları.** İçeriğin önemli bir kısmı üçüncü kişi kaynaklarından derlenir
ve **özet ile kaynak bağlantısı** biçiminde sunulur. Platform, kaynak içeriğin doğruluğundan,
güncelliğinden ve eksiksizliğinden sorumlu değildir; her içerikte kaynağı gösterilir ve
kaynağın kendisine yönlendirme yapılır. → [İçerik Kaynak ve Telif Politikası](/doctorium/icerik-politikasi)

**4.5. Prospektüs bilgisi.** İlaç bilgisi araması, **yürürlükteki resmî ürün bilgisine erişimi
kolaylaştırma** amacı taşır; ürün tanıtımı veya tavsiyesi değildir. Bağlayıcı olan, ilgili
otoritenin yayımladığı güncel metindir.

**4.6. Kongre ve etkinlik bilgileri.** Tarih, yer, son başvuru tarihi gibi bilgiler kaynak
kuruluşlardan derlenir ve değişebilir; bağlayıcı olan düzenleyicinin duyurusudur. **Platform,
etkinliklere ilişkin akreditasyon veya kredi bilgisi konusunda beyanda bulunmaz.**

**4.7. Süreklilik.** Platform, hizmeti kesintisiz sunmayı hedefler ancak bakım, güncelleme veya
teknik zorunluluk hâllerinde geçici kesinti yaşanabilir.

---

## 5. ÜCRET

**5.1.** Doctorium üyeliği Üye için **ücretsizdir.**

**5.2.** Platform, ileride ücretli hizmet sunması hâlinde bunu önceden duyurur; mevcut ücretsiz
işlevlerin ücretlendirilmesi hâlinde Üye'ye **kabul etmeme ve üyeliğini sona erdirme** hakkı
tanınır.

**5.3.** Üye, Doctorium'u **mesleki faaliyeti kapsamında** kullandığını kabul eder. Hizmet
ücretsiz olduğundan ve mesleki bilgilenme amacına yönelik sunulduğundan, bu sözleşme bir tüketici
işlemi niteliği taşımaz.

---

## 6. ÜYENİN YÜKÜMLÜLÜKLERİ

Üye:
   a) Platformu yalnız **mesleki bilgilenme** amacıyla kullanır;
   b) İçeriği sistematik biçimde kopyalamaz, çoğaltmaz, toplu indirme veya otomatik veri çekme
      (kazıma) yapmaz, üçüncü kişilere yeniden yayımlamaz;
   c) Hesabını başkasına kullandırmaz, giriş bilgilerini paylaşmaz;
   d) Platformun güvenliğini tehlikeye düşürecek işlemlerden kaçınır;
   e) Platform üzerinden **hasta verisi paylaşmaz** — Doctorium hasta verisi işlemeye yönelik
      bir ortam değildir;
   f) Yürürlükteki mevzuata ve mesleki etik kurallarına uygun davranır.

---

## 7. SPONSORLU İÇERİK VE ANKETLER

**7.1.** Sponsorlu içerik, tarafsız içerikten **ayırt edilebilir biçimde** "Sponsorlu" etiketi ve
sponsor kimliği ile yayımlanır; doğal içerik görünümü verilmez.

**7.2.** Sponsorlu içeriğin **mesleki profilinize göre kişiselleştirilmesi yalnız açık rızanızla**
yapılır; rıza vermemeniz veya geri almanız hâlinde sponsorlu içerik kişiselleştirilmeden
gösterilmeye devam eder. → sponsorlu içerik açık rıza metni (Tercihler sayfası)

**7.3.** Anketlere katılım gönüllüdür ve ayrı koşullara tabidir.
→ Anket Katılım Koşulları (anket kartında)

**7.4.** **Beşeri tıbbi ürün (ilaç) tanıtımı**, ayrı bir mevzuat çerçevesi ve yazılı sözleşme
olmaksızın Doctorium'da yayımlanmaz.

---

## 8. PUAN VE ÖDÜL PROGRAMI

Platform, katılımı teşvik amacıyla puan verebilir. **Puanlar parasal değer taşımaz, nakde
çevrilemez ve kazanılmış hak doğurmaz.** Program koşulları ayrıca düzenlenmiştir.
→ Puan ve Ödül Programı Koşulları (Ödüller sayfası)

---

## 9. FİKRİ MÜLKİYET

**9.1.** Platformun tasarımı, yazılımı, markası, logosu ve Platform tarafından üretilen özgün
içerik (derleme, özet, sınıflandırma) üzerindeki haklar Platform'a aittir.

**9.2.** Üye'ye, üyeliği süresince ve yalnız kişisel mesleki kullanım için, **devredilemez ve
münhasır olmayan** bir kullanım hakkı tanınır.

**9.3.** Üçüncü kişi kaynaklı içerik üzerindeki haklar ilgili hak sahiplerine aittir.

---

## 10. ÜYELİĞİN ASKIYA ALINMASI VE SONA ERMESİ

**10.1.** Üye, üyeliğini dilediği zaman **hesap silme** yoluyla sona erdirebilir. Silme talebi
üzerine kişisel verileri, saklama politikasında belirtilen istisnalar dışında silinir.

**10.2.** Platform, aşağıdaki hâllerde üyeliği **askıya alabilir veya sona erdirebilir**:
   a) Gerçeğe aykırı beyan veya sahte belge ile doğrulama,
   b) Madde 6'daki yükümlülüklerin ihlali,
   c) Platform güvenliğine yönelik girişim,
   d) Mesleki kimliğin geçerliliğini yitirmesi.

**10.3.** Askıya alma veya sona erdirme kararı Üye'ye **gerekçesiyle birlikte** bildirilir. Üye,
kararın bildiriminden itibaren **15 gün** içinde **bilgi@doctorium.tr** adresine itiraz edebilir. İtiraz, veri
sorumlusu temsilcisi tarafından değerlendirilir ve en geç **30 gün** içinde gerekçeli olarak
cevaplanır. Gerçeğe aykırı beyan veya sahte belge tespitine dayanan kararlarda itiraz, üyeliği
yeniden açmaz.

---

## 11. SORUMLULUĞUN SINIRI

**11.1.** Platform, madde 4'te belirtilen bilgi hizmetinin niteliği gereği, içeriğe dayanılarak
verilen klinik veya hukuki kararlardan sorumlu değildir.

**11.2.** Platform, üçüncü kişi kaynaklarına yapılan yönlendirmelerde hedef içerikten sorumlu
değildir.

**11.3.** Bu madde, Platform'un **kastından ve ağır ihmalinden** doğan sorumluluğunu ve emredici
hükümlerden doğan sorumluluğunu sınırlamaz.

**11.4.** Platform, Doctorium'da sunduğu derleme içerik bakımından 5651 sayılı Kanun anlamında
**içerik sağlayıcı** konumundadır; üyelerin ürettiği içeriği barındırmaz ve yer sağlayıcı değildir.
Sponsorlu içerikte Platform, içeriği "Sponsorlu" işaretiyle yayımlayan reklam yayıncısıdır; içeriğin
doğruluğu ve mevzuata uygunluğu Reklamverene aittir.

---

## 12. KİŞİSEL VERİLERİN KORUNMASI

Üye'nin kişisel verilerinin işlenmesine ilişkin esaslar **Aydınlatma Metni**'nde düzenlenmiştir.
→ [Aydınlatma Metni](/doctorium/aydinlatma)

---

## 13. DEĞİŞİKLİKLER

**13.1.** Platform bu sözleşmeyi güncelleyebilir. **Esaslı değişiklikler** yürürlüğe girmeden
önce Üye'ye bildirilir.

**13.2.** Üye, esaslı değişikliği kabul etmemesi hâlinde üyeliğini sona erdirebilir.

---

## 14. UYGULANACAK HUKUK VE YETKİ

**14.1.** Bu sözleşmeye **Türk hukuku** uygulanır.

**14.2.** Uyuşmazlıklarda **İzmir Mahkemeleri ve İcra Daireleri** yetkilidir.

---

## 15. YÜRÜRLÜK

Üye'nin elektronik ortamda onay vermesiyle yürürlüğe girer. Onay kaydı, onaylanan metnin özeti
(hash) ile birlikte zaman damgalı olarak saklanır.
`;
