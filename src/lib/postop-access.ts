...
export function recoveryClosed(r: { status: string; startedAt: Date; branch: string }): { closed: boolean; reason: CloseReason | null } {
  if (r.status === "COMPLETED") return { closed: true, reason: "MANUAL" };
  const elapsedDays = (Date.now() - new Date(r.startedAt).getTime()) / 86_400_000;
...
export async function caseRecoveryClosed(caseId: string): Promise<{ closed: boolean; reason: CloseReason | null }> {
  const r = await db.recovery.findUnique({
    where: { caseId },
    select: { status: true, startedAt: true, branch: true },
  });
