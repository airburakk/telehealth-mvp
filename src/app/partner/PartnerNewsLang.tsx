...
export function PartnerNewsLang({ current, languages }: { current: string; languages: string[] }) {
  ...
  return (
    <label className="inline-flex items-center gap-1.5 text-xs text-[var(--c-ink-2)]">
      {saving ? <Loader2 size={13} className="animate-spin" /> : <Languages size={13} />}
      <select
        value={current}
        onChange={(e) => change(e.target.value)}
        disabled={saving}
        className="rounded-lg border border-[var(--c-hairline)] bg-[var(--c-panel)] px-2 py-1 text-xs outline-none focus:border-[#34d399] disabled:opacity-60"
      >
