  ...
  return (
    <div dir={dir}>
      ...
      <div className="mb-6 flex items-start justify-between gap-3">
        <div>
          <p className="text-sm text-white/50">{t("Hoş geldiniz")}, {name}</p>
          <h1 className="mt-1 font-serif text-2xl font-bold tracking-tight text-[#F4F5F3]">{t("Nasıl ilerlemek istersiniz?")}</h1>
          <p className="mt-1 text-sm text-white/50">{t("Size uygun yolu seçin — seçiminizi daha sonra değiştirebilirsiniz.")}</p>
        </div>
        <PatientLangSelect lang={lang} onChange={setLang} dark />
      </div>

      {error && <div className="mb-4 rounded-lg bg-red-500/10 px-3 py-2 text-sm text-red-300 ring-1 ring-red-400/25">{error}</div>}
...
function ChoiceCard({ icon: Icon, title, desc, current, busy, disabled, onClick }: {
  ...
  return (
    <button type="button" onClick={onClick} disabled={disabled}
      className="group relative rounded-2xl border border-white/10 bg-[#161719] p-5 text-start transition hover:border-[#28C8D8]/50 disabled:opacity-60">
      {current && <span className="absolute end-4 top-4 rounded-full bg-[#28C8D8]/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-[#28C8D8]">{current}</span>}
      <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#28C8D8]/10 text-[#28C8D8]"><Icon size={20} /></span>
      <h2 className="mt-3 text-base font-semibold text-[#F4F5F3]">{title}</h2>
      <p className="mt-1 text-sm text-white/50">{desc}</p>
      <span className="mt-3 inline-flex items-center gap-1 text-sm font-semibold text-[#28C8D8]">
