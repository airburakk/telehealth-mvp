  ...
  return (
    <span className="inline-flex items-center gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star key={i} size={15} className={i < full ? "fill-amber-400 text-amber-400" : "text-[var(--c-ink-3)]"} />
      ))}
    </span>
  );
}
...
export default async function DoctorProfile({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    ...
    <div className="mx-auto max-w-4xl px-5 py-8">
      <Link href="/doktorlar" className="inline-flex items-center gap-1.5 text-sm text-[var(--c-ink-2)] hover:text-[var(--c-accent)]">
        <ArrowLeft size={16} /> Doktorlar
      </Link>

      {/* Hero */}
      <div className="mt-4 rounded-[22px] border border-[var(--c-hairline)] bg-[var(--c-panel)] p-6">
          ...
          <div className="flex-1">
            ...
            {d.rating != null && (
              <div className="mt-3 flex flex-wrap items-center gap-4">
                <span className="inline-flex items-center gap-1.5 text-sm"><Stars value={d.rating} /> <span className="font-semibold text-[var(--c-ink)]">{d.rating.toFixed(1)}</span> <span className="text-[var(--c-ink-3)]">({reviews.length} yorum)</span></span>
              </div>
            )}
            ...
            {badges.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-1.5">
                {badges.map((b) => {
                  ...
                  const style = BADGE_STYLE[b.key] ?? "bg-[var(--c-ink)]/5 text-[var(--c-ink-2)] ring-white/15";
                  return (
                    <span
                      key={b.key}
                    ...
                    >
                      ...
                      <span
                        role="tooltip"
                        className="pointer-events-none absolute bottom-full left-1/2 z-20 mb-1.5 hidden w-max max-w-[220px] -translate-x-1/2 rounded-lg bg-[var(--c-surface-2)] px-2.5 py-1.5 text-[11px] font-normal leading-snug text-[var(--c-ink)] shadow-lg ring-1 ring-white/10 group-hover:block"
                      >
                        {b.desc}
                      </span>
                    </span>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_300px]">
        <div className="space-y-5">
          <Card title="Hakkında">
            <p className="text-sm leading-relaxed text-[var(--c-ink-2)]">{bioText}</p>
          </Card>
              ...
              {reviews.map((r, i) => (
                <li key={i} className="rounded-2xl border border-[var(--c-hairline)] p-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-[var(--c-ink)]">{countryFlag(r.country)} {r.author}</span>
                    {/* "Doğrulanmış" çipi yalnız gerçek DB yorumlarında; üretilmiş içerik açıkça etiketlenir */}
                    {r.generated ? (
                      <span className="rounded-full bg-[var(--c-ink)]/10 px-2 py-0.5 text-[11px] font-medium text-[var(--c-ink-3)]">Örnek değerlendirme</span>
                    ) : (
...
function Card({ title, icon, children }: { title: string; icon?: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="rounded-[22px] border border-[var(--c-hairline)] bg-[var(--c-panel)] p-5">
      <div className="flex items-center gap-1.5 font-mono text-xs font-medium uppercase tracking-wide text-[var(--c-ink-3)]">{icon} {title}</div>
  ...
  return (
    <div className="rounded-2xl bg-[var(--c-surface)] p-3">
      <div className="flex items-baseline justify-between">
        ...
        <span className="text-lg font-bold text-[var(--c-ink)]">{valueText}</span>
      </div>
...
function Cred({ ok, label }: { ok: boolean; label: string }) {
  return (
    <li className="flex items-center gap-2">
      <CheckCircle2 size={15} className={ok ? "text-emerald-400" : "text-[var(--c-ink-3)]"} />
      <span className={ok ? "text-[var(--c-ink)]" : "text-[var(--c-ink-3)] line-through"}>{label}</span>
    </li>
  );
}
