...
//
//   · mode="close" → yalnız Doctorium üyeliği olan hesap (Aşama 1 doktoru / tıp öğrencisi / deneme).
...
//
// 🪤 2026-09-06 düzeltme: sponsor/anket/puan/ödül `audienceFlags`'te YALNIZ VERIFIED'a açıktır
// (öğrenci · deneme · kilitli hesapta bu yüzeyler hiç var olmadı) — panel bunu bilmediği için
// öğrenci bir hesaba da "anket yanıtlarınız kalır" gibi var olmayan verilerden bahsediyordu.
// `hadMarketingSurfaces` çağıran sayfadan gelir (`audienceFlags(audience).canSeeSurveys`); "leave"
// modu normalde her zaman Aşama 2 (VERIFIED) doktorudur ama `countClinicalTies` bunu VARSAYMAZ
// (bkz. lib/doctorium-membership.ts yorumu) — o yüzden leave modu da bu bayrağı kullanır.
...
function getCopy(mode: MembershipMode, hadMarketingSurfaces: boolean): CopyShape {
  if (mode === "leave") {
    return {
      ...
      removed: [
        ...
        "Takviminiz",
        ...(hadMarketingSurfaces ? ["Puanlarınız"] : []),
        "Akış tercihleriniz",
        "Doctorium bildirimleriniz",
        ...(hadMarketingSurfaces ? ["Sponsor içerik rızanız"] : []),
      ],
      warning: hadMarketingSurfaces
      ...
      keeps: (
        <>
          ...
          mezun belgeniz ve AURA klinik verileriniz.{hadMarketingSurfaces && (
            <> Anket yanıtlarınız ve tamamlanmış ödül kullanımlarınız kimlik bağı olmadan yerinde kalır.</>
          )}{" "}Doctorium&rsquo;a erişiminiz kapanır; dilediğiniz zaman yeniden üye olabilirsiniz.
        </>
      ),
      word: "ÇIK",
