...
import { congressById, isFollowingCongress, branchLabel } from "@/lib/doctorium";
...
export const metadata = { title: "Kongre" };

/**
 * Kongre bilgi kartı (v6.62, kullanıcı isteği) — her kongrenin kendi sayfası.
 *
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 py-8">
      <Link href="/doktor/doctorium?m=kongre" className="inline-flex items-center gap-1.5 text-sm text-[var(--c-ink-2)] hover:text-[var(--c-ink)]">
        <ArrowLeft size={15} /> Kongre Takvimi
      </Link>
      ...
      <div className="mt-4 overflow-hidden rounded-2xl border border-[var(--c-hairline)] bg-[var(--c-surface)]">
        ...
        <dl className="grid gap-x-6 gap-y-3 border-t border-[var(--c-hairline)] px-5 py-4 text-sm sm:grid-cols-2">
          ...
          {c.cmeCredit && <Field icon={<ShieldCheck size={13} />} label="Kredi">{c.cmeCredit}</Field>}
          {slugs.length > 0 && (
