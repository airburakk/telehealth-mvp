...
async function currentDoctoriumAccess(doctorId: string): Promise<boolean> {
  const d = await db.doctor.findUnique({ where: { id: doctorId }, select: { diplomaVerifiedAt: true, studentVerifiedAt: true, doctoriumOptOutAt: true, trialEndsAt: true } });
  return hasDoctoriumAccess(d ?? { diplomaVerifiedAt: null, studentVerifiedAt: null, doctoriumOptOutAt: null, trialEndsAt: null });
}
