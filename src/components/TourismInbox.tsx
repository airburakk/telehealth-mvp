...
// i18n (2026-07-17, Ray sonrası tur): statik etiketler ConsultGate deseniyle sunucuda çevrilir —
// tmap prop ile ilk boyama hasta dilinde. Bkz. lib/tourism-inbox-texts.ts + vaka/[caseId]/page.tsx.
// Doktor mesajı serbest metin olduğundan hâlâ TR (klinik-anlatı çeviri ayrı tur).
export function TourismInbox({
  ...
  country: string;
  outreaches: Outreach[];
  // Sunucuda hazırlanmış çeviri haritası; yoksa TR-kimlik fallback (geriye uyumlu).
  ...
  const fill = (s: string) => t(s).replace("{branch}", branchLabel).replace("{country}", country);
  ...
  async function respond(id: string, action: "accept" | "decline") {
    ...
    try {
      ...
      if (!r.ok) throw new Error(d.error || t("İşlem başarısız."));
      setItems((prev) => prev.map((o) => (o.id === id ? { ...o, status: action === "accept" ? "ACCEPTED" : "DECLINED" } : o)));
    } catch (e) {
      setErr(e instanceof Error ? e.message : t("Bir hata oluştu."));
    } finally {
      setBusyId(null);
    }
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start gap-3 rounded-3xl border border-[var(--c-accent)]/25 bg-[var(--c-accent)]/[0.08] p-5">
        ...
        <div>
          <h1 className="aura-display text-lg font-medium tracking-tight text-[var(--c-ink)]">{fill("Talebiniz {branch} doktorlarına iletildi")}</h1>
          <p className="mt-0.5 text-sm text-[var(--c-ink-2)]">
            {fill("{country} için {branch} branşındaki doktorlar talebinizi inceliyor. Size tanıtım mesajı gönderip video görüşme randevusu önerecekler. Gelen tekliflerden birini kabul ettiğinizde görüşme planlanır.")}
          </p>
        </div>
      </div>
      ...
      {accepted && (
        <div className="flex items-center gap-2 rounded-2xl border border-[var(--c-success)]/30 bg-[var(--c-success)]/10 p-4 text-sm font-medium text-[var(--c-success)]">
          <Check size={16} /> {t("Video görüşme teklifini kabul ettiniz. Randevu saatinde bu sayfadan görüşmeye katılabilirsiniz.")}
        </div>
      )}

      {items.length === 0 ? (
        <div className="rounded-2xl border border-[var(--c-hairline)] p-6 text-center text-sm text-[var(--c-ink-3)]">
          {t("Henüz doktor mesajı yok — talebiniz doktorlar tarafından inceleniyor.")}
        </div>
      ) : (
          ...
          {items.map((o) => (
            <li key={o.id} className="rounded-2xl border border-[var(--c-hairline)] bg-[var(--c-panel)] p-4 shadow-sm">
              <div className="flex items-center justify-between gap-2">
                ...
                {o.status === "ACCEPTED" && <span className="rounded-full bg-[var(--c-success)]/15 px-2.5 py-1 text-xs font-semibold text-[var(--c-success)]">{t("Kabul edildi")}</span>}
                {o.status === "DECLINED" && <span className="text-xs text-[var(--c-ink-3)]">{t("Reddedildi")}</span>}
              </div>
              ...
              {o.proposedAtLabel && (
                <div className="mt-2 inline-flex items-center gap-1.5 rounded-lg bg-[var(--c-accent)]/10 px-3 py-1.5 text-xs font-medium text-[var(--c-accent-strong)]">
                  <Video size={13} /> {t("Video görüşme önerisi")}: {o.proposedAtLabel}
                </div>
              )}
              {o.status === "SENT" && o.proposedAtLabel && !accepted && (
                <div className="mt-3 flex items-center gap-2">
                  <button onClick={() => respond(o.id, "accept")} disabled={busyId === o.id} className="inline-flex items-center gap-1.5 rounded-lg bg-[var(--c-accent)] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[var(--c-accent-strong)] disabled:opacity-60">
                    {busyId === o.id ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />} {t("Kabul et")}
                  </button>
