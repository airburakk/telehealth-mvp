...
export default function ProcedureSelector({
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            <span className="font-medium text-[#0A7D77]">{branchLabel}</span> branşındaki işlemlerden yaptıklarınızı seçin;
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-lg bg-[#0E9E97] px-3.5 py-2 text-sm font-semibold text-white hover:bg-[#0A7D77] disabled:cursor-not-allowed disabled:opacity-40"
          >
      ...
      {showOther && (
        <div className="mt-3 rounded-xl border border-dashed border-teal-300 bg-teal-50/40 p-3">
      ...
      {allItems.length === 0 ? (
        ...
        <div className="mt-4 space-y-2">
          ...
          {groups.length === 0 && <div className="rounded-xl border border-dashed border-slate-300 p-4 text-center text-sm text-slate-400">Aramayla eşleşen işlem yok.</div>}
        </div>
      )}
    </div>
  );
...
function ProcRow({
  ...
  const color = floor > 0 ? hueFor(cur, floor, ceil) : "#0E9E97";
  const step = floor > 0 ? Math.max(1, Math.round((ceil - floor) / 100)) : 1;

  return (
    <li className="px-4 py-3">
      <label className="flex cursor-pointer items-start gap-3">
        <span className={`mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded border ${selected ? "border-[#0E9E97] bg-[#0E9E97] text-white" : "border-slate-300 bg-white"}`}>
