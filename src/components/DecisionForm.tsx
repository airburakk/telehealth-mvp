  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex items-center gap-2">
        <Gavel size={18} className="text-[#101010]" />
      ...
      <div className="mt-4 space-y-4">
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {Object.entries(VERDICTS).map(([k, v]) => (
              <button key={k} onClick={() => setVerdict(k)} className={`rounded-lg border px-2 py-2 text-sm font-medium transition ${verdict === k ? "border-[#14C3D0] bg-[#14C3D0] text-[#101010]" : "border-slate-200 text-slate-600 hover:border-slate-300"}`}>
                {v.label}
              </button>
            ))}
          </div>
        ...
        <label className="block">
          ...
          <select value={action} onChange={(e) => setAction(e.target.value)} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-[#14C3D0]">
        ...
        {action === "REFUND_PARTIAL" && bookingTotal && (
          <label className="block">
            ...
            <input type="number" value={refundAmount} min={0} max={bookingTotal} onChange={(e) => setRefundAmount(Number(e.target.value))} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-[#14C3D0]" />
          </label>
        )}

        <label className="block">
          <span className="mb-1.5 block text-sm font-medium text-slate-700">Gerekçe</span>
          <textarea value={rationale} onChange={(e) => setRationale(e.target.value)} rows={3} placeholder="Kurul kararının gerekçesi…" className="w-full resize-none rounded-lg border border-slate-300 p-2.5 text-sm outline-none focus:border-[#14C3D0]" />
        </label>

        <label className="block">
          <span className="mb-1.5 block text-sm font-medium text-slate-700">İmzalayan kurul üyesi</span>
          <select value={decidedBy} onChange={(e) => setDecidedBy(e.target.value)} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-[#14C3D0]">
