...
// Hasta teklifi aksiyonları — onayla (Escrow), PDF/yazdır, reddet.
// Yazdırırken (print:hidden) bu çubuk gizlenir → temiz PDF belgesi.
export function OfferActions({ bookingId, total }: { bookingId: string; total: string }) {
  const router = useRouter();
  const [busy, setBusy] = useState<null | "approve" | "decline">(null);
  const [declined, setDeclined] = useState(false);

  async function respond(action: "approve" | "decline") {
    if (action === "decline" && !confirm("Teklifi reddetmek istediğinize emin misiniz?")) return;
    setBusy(action);
    try {
  ...
  return (
    <div className="print:hidden space-y-2">
      ...
      >
        ...
        Teklifi onayla · Escrow güvencesiyle ({total})
      </button>
      <div className="flex gap-2">
        ...
        >
          {busy === "decline" ? <Loader2 size={15} className="animate-spin" /> : <X size={15} />} Reddet
        </button>
      </div>
    </div>
  );
