  ...
  return (
    <div className="w-full max-w-sm">
      <div className="mb-6 flex flex-col items-center text-center">
        ...
        <h1 className="mt-3 font-serif text-xl font-bold tracking-tight text-[#F4F5F3]">{title}</h1>
      ...
      <div className="rounded-[22px] border border-white/10 bg-[#161719] p-6">
        ...
        <form onSubmit={(e) => { e.preventDefault(); login(); }} className="space-y-3">
          <label className="block">
            ...
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="ornek@air.test" className="w-full rounded-lg border border-white/10 bg-[#1E1F22] px-3 py-2 text-sm text-[#F4F5F3] outline-none placeholder:text-white/25 focus:border-[#28C8D8]" />
          </label>
          <label className="block">
            <span className="mb-1.5 block text-sm font-medium text-white/70">Parola</span>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••" className="w-full rounded-lg border border-white/10 bg-[#1E1F22] px-3 py-2 text-sm text-[#F4F5F3] outline-none placeholder:text-white/25 focus:border-[#28C8D8]" />
          </label>
          ...
          {needsVerify && (
            <div className="rounded-lg bg-amber-500/10 px-3 py-2 text-sm text-amber-300 ring-1 ring-amber-400/25">
              {resendMsg ? (
                ...
                <button type="button" onClick={resendVerification} className="font-semibold text-[#28C8D8] underline-offset-2 hover:underline">
        ...
        {quick.length > 0 && (
          <>
            <div className="my-4 flex items-center gap-3 text-xs text-white/40">
              <span className="h-px flex-1 bg-white/10" /> Hızlı demo girişi <span className="h-px flex-1 bg-white/10" />
            </div>
            ...
            <div className={`grid gap-2 ${quick.length > 1 ? "grid-cols-2" : "grid-cols-1"}`}>
              {quick.map((q) => {
                const Icon = q.icon;
                return (
                  <button key={q.email} onClick={() => login(q.email, "1234")} disabled={loading} className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-white/10 bg-[#1E1F22] px-3 py-2 text-sm font-medium text-white/75 hover:border-[#28C8D8]/40 hover:text-[#F4F5F3] disabled:opacity-60">
                    <Icon size={15} /> {q.label}
                  </button>
                );
              })}
            </div>
            <p className="mt-3 text-center text-[11px] text-white/40">Demo parolası: <span className="font-mono">1234</span></p>
          </>
        )}
      </div>
