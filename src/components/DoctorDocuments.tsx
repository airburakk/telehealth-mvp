              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 text-sm font-semibold text-[var(--c-ink)]">
                  ...
                  {required ? (
                    ...
                    <span className="rounded-full bg-[var(--c-ink)]/10 px-2 py-0.5 text-[10px] font-bold uppercase text-[var(--c-ink-2)]">İhtiyari</span>
                  )}
                </div>
            ...
            {type === "MMSS" && (
              <div className="mt-3 rounded-2xl bg-[var(--c-panel)] p-3 ring-1 ring-white/10">
                ...
                <div className="mt-2 grid gap-2 sm:grid-cols-2">
                  ...
                  <Field label="Teminat limiti">
                    <div className="flex gap-1.5">
                      ...
                      <select value={currency} onChange={(e) => { setCurrency(e.target.value); setMmssSaved(false); }} className="rounded-lg border border-[var(--c-hairline)] px-2 text-sm">
                ...
                <button onClick={saveMmss} disabled={savingMmss || !insurer.trim() || coverageLimit === "" || (!policyNo.trim() && !initialMmss.policyNoSet)}
                  className="mt-2.5 inline-flex items-center gap-1.5 rounded-lg bg-[var(--c-accent)] px-3 py-1.5 text-xs font-semibold text-[var(--c-bg)] hover:bg-[var(--c-accent-strong)] disabled:opacity-50">
...
const inputCls = "w-full rounded-lg border border-[var(--c-hairline)] px-2.5 py-1.5 text-sm focus:border-[var(--c-accent)] focus:outline-none";

function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="mb-1 block text-[11px] font-medium text-[var(--c-ink-2)]">{label}</span>
      {children}
    </label>
  );
}
