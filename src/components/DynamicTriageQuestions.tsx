              ...
              <div className="flex flex-wrap gap-1">
                {Array.from({ length: 11 }, (_, i) => String(i)).map((n) => (
                  <button key={n} type="button" onClick={() => set(q.label, val === n ? "" : n)}
                    className={`h-8 w-8 rounded-md text-sm font-medium transition ${val === n ? "bg-[#0f2a4a] text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
                    {n}
                  </button>
                ))}
              </div>
...
function Chip({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button type="button" onClick={onClick}
      className={`rounded-full border px-3 py-1.5 text-sm transition ${active ? "border-[#0f2a4a] bg-[#0f2a4a] text-white" : "border-slate-300 bg-white text-slate-600 hover:border-[#0f2a4a]/40 hover:bg-slate-50"}`}>
      {children}
    </button>
  );
}
