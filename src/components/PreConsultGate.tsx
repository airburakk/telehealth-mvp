...
export function PreConsultGate({ onCleared, t = (s) => s }: { onCleared: (b: Billing) => void; t?: (s: string) => string }) {
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-[#161719] p-6 shadow-sm">
      ...
      <div className="mt-5">
        <div className="text-xs font-semibold uppercase tracking-wide text-white/50">{t("Ödeme yöntemi")}</div>
        <div className="mt-2 grid gap-2 sm:grid-cols-2">
          <button
            type="button"
            onClick={() => { setError(""); setMethod("card"); }}
            className={`rounded-2xl border-2 p-3.5 text-start transition-colors ${method === "card" ? "border-[#28C8D8] bg-[#28C8D8]/5" : "border-white/10 hover:border-white/25"}`}
          >
            <CreditCard size={20} className="text-[#F4F5F3]" />
            <div className="mt-1.5 text-sm font-semibold text-[#F4F5F3]">{t("Kartla öde")}</div>
            <div className="text-xs text-white/50">${CONSULT_FEE_USD} {t("ödeyerek devam edin")}</div>
          </button>
          <button
            type="button"
            onClick={() => { setPolicyMsg(""); setMethod("insurance"); }}
            className={`rounded-2xl border-2 p-3.5 text-start transition-colors ${method === "insurance" ? "border-[#28C8D8] bg-[#28C8D8]/5" : "border-white/10 hover:border-white/25"}`}
          >
            <ShieldCheck size={20} className="text-[#F4F5F3]" />
            <div className="mt-1.5 text-sm font-semibold text-[#F4F5F3]">{t("Sigortam var")}</div>
      ...
      {method === "insurance" ? (
        <div className="mt-4">
          <p className="text-sm text-white/50">{t("Poliçe numaranızı girin; görüşme kapsamı kontrol edilsin.")}</p>
