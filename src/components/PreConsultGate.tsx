...
const PRIMARY = "inline-flex items-center justify-center gap-1.5 rounded-lg bg-[#0E9E97] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#0A7D77] disabled:opacity-50";
const BACK = "inline-flex items-center gap-1.5 rounded-lg px-3.5 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100";
const INPUT = "w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-[#0E9E97] focus:outline-none";
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1 text-xs">
        {steps.map((label, i) => (
          <span key={i} className={`inline-flex items-center gap-1.5 ${i <= stepIndex ? "font-semibold text-[#0A3F39]" : "text-slate-400"}`}>
            <span className={`grid h-5 w-5 place-items-center rounded-full text-[10px] ${i < stepIndex ? "bg-emerald-500 text-white" : i === stepIndex ? "bg-[#0E9E97] text-white" : "bg-slate-200 text-slate-500"}`}>
      ...
      <div className="mt-5">
        {phase === "info" && (
          <div>
            <div className="flex items-center gap-3">
              <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-[#0E9E97] text-white"><Video size={22} /></span>
              <div>
                <h2 className="text-lg font-bold text-[#0A3F39]">{t("Uzman görüşmesi — ön bilgilendirme")}</h2>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <div className="rounded-xl border border-slate-200 p-4">
                ...
                <div className="mt-1 text-2xl font-bold text-[#0A3F39]">${CONSULT_FEE_USD}</div>
              ...
              <div className="rounded-xl border border-slate-200 p-4">
                ...
                <div className="mt-1 text-2xl font-bold text-[#0A3F39]">{CONSULT_DURATION_TEXT}</div>
        ...
        {phase === "insurance" && (
          <div>
            <h2 className="text-lg font-bold text-[#0A3F39]">{t("Sigorta durumu")}</h2>
            <p className="mt-1 text-sm text-slate-500">{t("Bu görüşmeyi kapsayan bir sağlık sigortanız var mı?")}</p>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <button onClick={() => { setPhase("policy"); setPolicyMsg(""); }} className="rounded-xl border-2 border-slate-200 p-4 text-left transition-colors hover:border-[#0E9E97] hover:bg-[#0E9E97]/5">
                <ShieldCheck size={22} className="text-[#0A3F39]" />
              ...
              <button onClick={() => { setError(""); setPhase("payment"); }} className="rounded-xl border-2 border-slate-200 p-4 text-left transition-colors hover:border-[#0E9E97] hover:bg-[#0E9E97]/5">
                <CreditCard size={22} className="text-[#0A3F39]" />
        ...
        {phase === "policy" && (
          <div>
            <h2 className="text-lg font-bold text-[#0A3F39]">{t("Sigorta poliçesi doğrulama")}</h2>
        ...
        {phase === "payment" && (
          <div>
            <h2 className="text-lg font-bold text-[#0A3F39]">{t("Ödeme")}</h2>
