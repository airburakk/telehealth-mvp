...
// Hasta nav kararı: PATIENT = Vakalarım · Post Op · Paylaşımlarım (Triyaj/Pro Bono/Doktorlar kalktı);
...
describe("navItemsFor", () => {
  it("PATIENT (journey yok): Vakalarım + Post Op + Paylaşımlarım; Triyaj/Pro Bono/Doktorlar YOK", () => {
    const h = hrefs("PATIENT", null);
    expect(h).toEqual(["/vakalarim", "/takip", "/paylasimlarim"]);
    expect(h).not.toContain("/triyaj");
    expect(h).not.toContain("/pro-bono/basvur");
  ...
  it("DOCTOR: değişmedi (Doktor, Post-Op, Pro Bono, Profilim); hasta sekmeleri yok", () => {
    expect(hrefs("DOCTOR")).toEqual(["/doktor", "/doktor/takip", "/doktor/pro-bono", "/doktor/profil"]);
  });
  ...
  it("COORDINATOR: Operasyon + Doktor + Post-Op + Pro Bono", () => {
    expect(hrefs("COORDINATOR")).toEqual(["/operasyon", "/doktor", "/doktor/takip", "/doktor/pro-bono"]);
  });
