...
| `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN` | ⛅ | **Upstash Redis** — dağıtık rate-limit (`lib/rate-limit.ts`: login 10/5dk/IP · paylaşım-şifre 10/5dk/IP+link · AI-SOAP 20/dk/kullanıcı · AI-triyaj 20/dk/kullanıcı [v6.61]). **Boşsa:** in-memory yedek (kırılmaz; serverless'ta instance-başına en-iyi-çaba). Upstash hatasında fail-open → in-memory. Kur: console.upstash.com → Redis Database → REST API iki değeri buraya + Vercel (Production+Preview, Sensitive). Vercel Marketplace `KV_REST_API_*` adları da tanınır |
...
| `RESEND_API_KEY` | ⛅→✅ | E-posta gönderimi + **e-posta doğrulama** (v5.6, `lib/email.ts`). Yeni e-posta kayıtları doğrulama bağlantısı almadan giriş yapamaz (mevcut/demo muaf; Google doğrulanmış sayılır). ⚠️ **ÜRETİMDE ARTIK ZORUNLU (v6.61):** kapı eskiden `isEmailConfigured()`'a bağlıydı — anahtar kaldırılırsa doğrulama zorunluluğu da kalkıyordu (fail-open). Artık üretimde kapı HER HÂLÜKÂRDA açık; anahtar yoksa doğrulanmamış hesaplar giremez **ve** `email-provider-missing` alarmı düşer. Geliştirmede eski esneklik sürer. Kur: resend.com → API Key (Sensitive; sır aktarımı bash `printf` ile — CRLF tuzağı) |
...
| `ALERT_EMAIL` | ⛅→✅ | Kritik alarm e-posta alıcısı (`lib/alerts.ts` — onam/denetim zinciri, KEK, decrypt kümesi, 12 cron'un başarısızlığı, e-posta sağlayıcı eksikliği). Boşsa alarmlar yalnız `[ALERT]` log satırı. ✅ **2026-09-12 üretimde AÇIK — İKİ Vercel projesine AYRI girildi** (telehealth-mvp + doctorium; env devralınmaz, `npx vercel env ls production` ikisinde de gösterir). Konu satırı marka etiketi taşır (`[AURA ALARM]` / `[DOCTORIUM ALARM]`, gövdede `Proje:`). Tatbikat: `/admin` → "Alarm kanalı" → "Test alarmı gönder" (`POST /api/admin/alarm-test`, aynı olay 30 dk'da bir). Config (Sensitive DEĞİL) — adres sır değildir, panelde görünür kalır |
